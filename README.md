...
```bash
sample-aws-sam-lambda$ sam build
```
...
```bash
sample-aws-sam-lambda$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
sample-aws-sam-lambda$ sam local start-api
sample-aws-sam-lambda$ curl http://localhost:3000/
```
...
```bash
sample-aws-sam-lambda$ sam logs -n HelloWorldFunction --stack-name sample-aws-sam-lambda --tail
```
...
```bash
sample-aws-sam-lambda$ cd hello-world
...
```bash
aws cloudformation delete-stack --stack-name sample-aws-sam-lambda
```
